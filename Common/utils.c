...
uchar countAvailProc(uchar detectID)
{
	...
	for(i=0; i<n; i++) {
		if(sv->vcpu_base[i+startingVirtCoreID].app_id == detectID && sv->vcpu_base[i+startingVirtCoreID].cpu_state > CPU_STATE_INIT)
